...
- 音声の明示的なオフライン保存、容量上限、端末内コピーの削除
- 選択した音声をWear OSへ転送し、WatchとBluetoothイヤホンだけで再生（実機確認手順は下記）
- 同名のLRC歌詞をスマホ・Watchで再生時刻に合わせて表示、音声と一緒にオフライン保存（[配置と使い方](docs/LYRICS.md)）
- ファイルごとのWatch同期対象をスマホに記憶し、手動でまとめて転送
- Watchのサムネイル一覧、再生位置のシークバー、メディア音量調整

Pixel Watch連携の使い方・対応範囲・検証状況は [Watch Offline Audio](docs/WATCH_AUDIO.md) を参照。
